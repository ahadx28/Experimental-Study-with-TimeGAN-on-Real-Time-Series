...
def train_step(x):
    with tf.GradientTape() as tape:
        h = embedder(x, training=True)              # (b, T, hidden)
        x_tilde = recovery(h, training=True)        # (b, T, D)
        loss = mse(x, x_tilde)
